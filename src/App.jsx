...
const apiBaseUrl = "https://escola-musica-api.onrender.com";
  ...
  return (
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <h1>Escola Musica</h1>
        <p className="login-slogan">Educação de qualidade</p>
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Aula</li>
...
function PainelProfessor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "instrumentos", "bio", "disponivel", "telefone", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "instrumentos", "bio", "disponivel", "telefone"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>instrumentos</b><span>{item["instrumentos"] ?? "—"}</span></div>
      ...
      <label className="field-label">instrumentos</label>
      <input type="text" value={form["instrumentos"] ?? ""} onChange={e => setForm({...form, instrumentos: e.target.value})} />
...
function PainelAluno({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "dataNascimento", "telefone", "instrumento", "nivel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "dataNascimento", "telefone", "instrumento", "nivel"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>telefone</b><span>{item["telefone"] ?? "—"}</span></div>
            <div className="item-field"><b>instrumento</b><span>{item["instrumento"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["instrumento"] ?? ""} onChange={e => setForm({...form, instrumento: e.target.value})} />
...
function PainelAula({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["professorId", "alunoId", "instrumento", "dataHora", "duracao", "valor", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/aulas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/aulas/" + editId : apiBaseUrl + "/api/aulas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["professorId", "alunoId", "instrumento", "dataHora", "duracao", "valor", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["professor", "aluno"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/aulas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["professorId"] ?? "Aula")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/aulas/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["professorId"] ?? "Aula")}</div>
      ...
      <input type="text" value={form["alunoId"] ?? ""} onChange={e => setForm({...form, alunoId: e.target.value})} />
      <label className="field-label">instrumento</label>
      <input type="text" value={form["instrumento"] ?? ""} onChange={e => setForm({...form, instrumento: e.target.value})} />
...
function PainelMatricula({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoId", "professorId", "instrumento", "nivel", "dataInicio", "mensalidade", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["alunoId", "professorId", "instrumento", "nivel", "dataInicio", "mensalidade", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno", "professor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["instrumento"] ?? "Matricula")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["instrumento"] ?? "Matricula")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>alunoId</b><span>{item["alunoId"] ?? "—"}</span></div>
            <div className="item-field"><b>professorId</b><span>{item["professorId"] ?? "—"}</span></div>
            <div className="item-field"><b>nivel</b><span>{item["nivel"] ?? "—"}</span></div>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Escola Musica<span className="dot">.</span></div>
        ...
        <button className={"nav-btn" + (aba === "Aula" ? " active" : "")} onClick={() => setAba("Aula")}><span className="nav-ico">🗂️</span>Aula</button>
      ...
      {aba === "Aula" && <PainelAula token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
